...
class Payment {
  ...
  /**
   * Request transfers from wechat
   * @param {object} info:
   * {
   *   partner_trade_no: '123',
   *   openid: '3210', 
   *   check_name: 'NO_CHECK', 
   *   amount: '100', 
   *   desc: '理赔'
   ...
   */
  transfers(info) {
    const data = this.generateSignature(this.mergeParamsFix(info));
    return utils
      .buildXML(data)
      .then(xmlData => {
      ...
      .catch(reason => {
        debug('request transfers operation failed!');
        return Promise.reject(reason);
      });
  }
  ...
  //   企业付款用
  mergeParamsFix(customParams, signType) {
    const wechatConfig = this.wechatConfig;
    return Object.assign(
      {
        mchid: wechatConfig.merchantId,
